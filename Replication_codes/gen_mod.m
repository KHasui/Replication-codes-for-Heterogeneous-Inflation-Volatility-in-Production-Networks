function modfile_name = gen_mod(ALPHA_,paramsvec,matscell,Gtype,stdevvals)
...
%             0 for emprical graph (Benchmark case but named different),
...
if nargin < 5, stdevvals = ones(paramsvec(6),2); end % stderr 1 for e_z(s) and e_ep(s).
...
GAMs = matscell{2};
    ...
    elseif Gtype == 4 % Line (Cycle) Graph
        ...
        G   = matscell{3};
        upstreamness_ = springRank(G);

        [~,rank_order] = sort(upstreamness_,'descend');
        GG = zeros(S,S);
        for iu = 1:S-1
            ind_supply = rank_order(iu);
            ind_use    = rank_order(iu+1);
            GG(ind_supply,ind_use) = 1;
        end
        GG(rank_order(S),rank_order(1)) = 1;
