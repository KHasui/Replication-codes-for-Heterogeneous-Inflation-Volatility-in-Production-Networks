function [modfile_name, folder_name, loadmat_name, savemat_name]...
    = gen_mod_inter(ALPHA_,paramsvec,matscell,G,CHIu,CHIhub,indCHIu,indCHIhub,U,outstrength_,length_,hub_degree,stdevvals)
...
if nargin < 12, hub_degree = 0.2; end % default value of hub_share is 0.2.
if nargin < 13, stdevvals = ones(paramsvec(6),2); end % stderr 1 for e_z(s) and e_ep(s).
...
% hubstructure
CHIhub_i = CHIhub(indCHIhub);

[~, ind_descend_hub] = sort(outstrength_,'descend');

Nhub = max(1,round(hub_degree*S));
hub_index = false(S,1);
hub_index(ind_descend_hub(1:Nhub)) = true;
Whub = repmat(double(hub_index),1,S);


% upstreamness
CHIu_i = CHIu(indCHIu);
[~, ind_descend_u] = sort(U,'descend');

data_index = 1:S;
Nstage = 4;
[data_stage, ~] = discretize(data_index, Nstage);

[~, ind_for_stage] = sort(ind_descend_u,'ascend');
...
GG_ = G.*exp( CHIu_i*Wu + CHIhub_i*Whub);
GG  = GG_./sum(GG_,1);
GGT = GG';
...
modfile_name = ['mod_inter_chain',num2str(length_),'_chiu',num2str(indCHIu),'_chihub',num2str(indCHIhub),'.mod'];
folder_name  = ['mod_inter_chain',num2str(length_),'_chiu',num2str(indCHIu),'_chihub',num2str(indCHIhub),'/Output'];
loadmat_name = ['mod_inter_chain',num2str(length_),'_chiu',num2str(indCHIu),'_chihub',num2str(indCHIhub),'_results.mat'];
savemat_name = ['result_mod_inter_chain',num2str(length_),'_chiu',num2str(indCHIu),'_chihub',num2str(indCHIhub),'.mat'];
...
% Display
disp([' Dynare mod file is produced with a chain degree: chi_ = ',num2str(CHIhub_i)])
