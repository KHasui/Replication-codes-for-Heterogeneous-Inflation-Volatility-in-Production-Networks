function [modfile_name, folder_name, loadmat_name, savemat_name]...
    = gen_mod_hub(ALPHA_,paramsvec,matscell,G,CHI_,indCHI,outstrength_,length_,hub_degree,stdevvals)
...
if nargin < 9, hub_degree = 0.2; end % default value of hub_share is 0.2.
if nargin < 10, stdevvals = ones(paramsvec(6),2); end % stderr 1 for e_z(s) and e_ep(s).
...
[~, ind_descend] = sort(outstrength_,'descend');

Nhub = max(1,round(hub_degree*S));
hub_index = false(S,1);
hub_index(ind_descend(1:Nhub)) = true;
W = repmat(double(hub_index),1,S);
...
modfile_name = ['model_network_hub_',num2str(length_),'_chi_',num2str(indCHI),'.mod'];
folder_name  = ['model_network_hub_',num2str(length_),'_chi_',num2str(indCHI),'/Output'];
loadmat_name = ['model_network_hub_',num2str(length_),'_chi_',num2str(indCHI),'_results.mat'];
savemat_name = ['result_model_network_hub_',num2str(length_),'_chi_',num2str(indCHI),'.mat'];
