function table_estim_result_group(filename, estim_result_mat,estim_result_cell,...
...
%tabname  =   fopen('Result_run03_estimation_table_tex.txt','w');
    ...
    fprintf(txtfilename,'\n    \\begin{tabular}{cccl}');
    fprintf(txtfilename,'\n        \\toprule');
    %fprintf(txtfilename,'\n        Group & $\\sigma_{z(s)}$ \\ (S.E.) & $\\sigma_{e(s)}$ \\ (S.E.) & BEA code & Sector Name \\\\');
    fprintf(txtfilename,'\n        Group & $\\sigma_{z(s)}$ \\ (S.E.) & $\\sigma_{e(s)}$ \\ (S.E.) & Sector Name \\\\');
    fprintf(txtfilename,'\n        \\hline');
    gind_old = 0;
    for j = 1:nj
        ...
        gind_ = group_ascend(j);
        if gind_ ~= gind_old
            gind_plot = num2str(gind_);
            sigz_plot = [' $ ',num2str(sigez_ascend(j),formatSpec_),' '];
            SEz_plot  = [' \\ (',num2str(SEz_ascend(j),formatSpec_),') $'];
            sigp_plot = [' $ ',num2str(sigep_ascend(j),formatSpec_),' '];
            SEp_plot  = [' \\ (',num2str(SEp_ascend(j),formatSpec_),') $'];
        else
            gind_plot = ' ';
            sigz_plot = ' ';
            SEz_plot  = ' ';
        ...
        gind_old = gind_;
        fprintf(txtfilename,['\n        ',textc,'    ',char(gind_plot) ,...
            ' & ',...
            sigz_plot, SEz_plot,...
            ...'$\underset{(',num2str(sigez_ascend(j)),')}{',num2str(SEz_ascend(j)),'}$',...
            ' & ',...
            sigp_plot, SEp_plot,...% ' & ',... % char(sector_code_ascend(j)),...
            ' & ',...
            char(sector_name_ascend(j)),...'$\underset{(',num2str(sigep_ascend(j)),')}{',num2str(SEp_ascend(j)),'}$',...
            '\\\\']);
